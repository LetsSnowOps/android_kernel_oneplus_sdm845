...
# V5.2E single hardening change
"$ROOT/scripts/config" --file "$OUT/.config" --enable STRICT_KERNEL_RWX

# V5.2E: enforce W^X permissions on loadable modules
"$ROOT/scripts/config" --file "$OUT/.config" --enable STRICT_MODULE_RWX

# V5.2E: zero memory when returned to allocator
"$ROOT/scripts/config" --file "$OUT/.config" --enable INIT_ON_FREE_DEFAULT_ON
